...
export function handleBarcodeClick(entry: BarcodeEntry) {
  if (entry.valueType === "URL" && entry.displayValue.startsWith("http"))
    Browser.open({ url: entry.displayValue });
...
export const allSelected = computed(
  () =>
    filteredBarcodes.value.length > 0 &&
    selectedIds.value.length === filteredBarcodes.value.length
);

export function toggleSelectAll() {
  allSelected.value
    ? (selectedIds.value = [])
    : (selectedIds.value = filteredBarcodes.value.map((b) => b.id));
}

export const requestDeleteSelected = () =>
...
export const filterActive = computed(() => {
  const active = activeValueTypes.value;
  const sel = selectedValueTypes.value;
